    ...
    <v-container class="stackContainer mt-4 pt-0">
      ...
      >
        <stack-item v-for="(image, i) in images" :key="i" class="stackItems">
          <v-container class="pa-0 pb-0 imageContainer" @click="$store.commit('resources/setLightbox',{index: i, type: 'gallery'})">
...
export default {
  ...
  computed: {
    images(){
      return this.$store.state.resources.gallery.images
    },
    ...mapState({
